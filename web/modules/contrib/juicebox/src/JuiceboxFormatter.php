...
class JuiceboxFormatter implements JuiceboxFormatterInterface, TrustedCallbackInterface {
  ...
  public function runCommonBuild(JuiceboxGalleryInterface $gallery, array $settings, $data = NULL) {
    ...
    // Also attempt to translate interface strings. Use isset/empty checks
    // to guard against missing configuration keys.
    if (isset($global_settings['translate_interface']) && $global_settings['translate_interface']) {
      $base_string = isset($global_settings['base_languagelist']) ? $global_settings['base_languagelist'] : '';
  ...
  protected function styleImage(FileInterface $file, $style, $check_compatible = TRUE) {
    ...
    // If the item is compatible, style it.
      ...
      if ($style == 'juicebox_multisize') {
        $sizes = [
          'smallImageURL' => isset($global_settings['juicebox_multisize_small']) ? $global_settings['juicebox_multisize_small'] : '',
          'imageURL' => isset($global_settings['juicebox_multisize_medium']) ? $global_settings['juicebox_multisize_medium'] : '',
          'largeImageURL' => isset($global_settings['juicebox_multisize_large']) ? $global_settings['juicebox_multisize_large'] : '',
        ];
      }
