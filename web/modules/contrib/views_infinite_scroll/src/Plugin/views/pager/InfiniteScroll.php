...
class InfiniteScroll extends SqlBase {
  ...
  public function summaryTitle() {
    ...
    // Ensure we pass a scalar count to the translation placeholders. When
    // `items_per_page` is NULL (for example when no pager is configured), the
    // translation system will attempt to escape NULL and raise a TypeError.
    $count = isset($this->options['items_per_page']) ? (int) $this->options['items_per_page'] : 0;

    return $this->formatPlural($count, '@action, @count item', '@action, @count items, @pages',
      [
        '@action' => $action,
        '@count' => $count,
        '@pages' => $pages,
